...
OUTPUT_COLUMNS = [
    "patient_id", "episode_id", "operation_date", "note_date", "days_post_op",
    "procedure_code", "procedure_description", "procedure_type", "hospital_site",
    "processing_mode", "ssi_classification", "p_none", "p_superficial", "p_deep",
    "p_organ_space", "confidence_zone", "review_required", "extracted_entities",
    "entity_snippets", "ecdc_window_flag", "icd10_codes",
]
...
def filter_mdt_review(df: pd.DataFrame) -> pd.DataFrame:
    ...
    if prob_cols:
        ...
        review_df = review_df.sort_values("_max_ssi", ascending=False).drop(columns=["_max_ssi"])
